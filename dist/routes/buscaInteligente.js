...
const escapeSqlString_1 = require("../utils/escapeSqlString");
const trabalhosRelacionados_1 = require("../utils/trabalhosRelacionados");
...
router.post("/buscar-inteligente", async (req, res) => {
    ...
    try {
        ...
        // 3. NORMALIZAR QUERY
        // ==========================================================
        const textoQuery = (0, normalizaTexto_1.normalizar)(query);
        // ==========================================================
        // 4. GERAR EMBEDDING DA QUERY
        // ==========================================================
        const queryEmbedding = await (0, embedding_service_1.generateEmbedding)(textoQuery);
        /*
    ...
    WITH base AS (
        ...
        SELECT
            ...
            ts_rank(
                ...
                plainto_tsquery(
                    'portuguese',
                    ${(0, escapeSqlString_1.escapeSqlString)(textoQuery)}
                )
            ) AS lexical_score
...
`);
        ...
        // ==========================================================
        const ids = resultadosRaw.map((item) => Number(item.id));
        let trabalhosRelacionados = [];
        if (ids.length > 0) {
        ...
        const resultados = resultadosRaw.map((item) => {
            const trabalho = trabalhosRelacionados.find((t) => t.id === Number(item.id));
            return {
                id: Number(item.id),
                titulo: item.titulo,
                resumo: item.resumo,
                ...
                semanticNormalized: Number(item.semantic_normalized),
                lexicalNormalized: Number(item.lexical_normalized),
        ...
        // 8. GARANTIR ORDEM
        // ==========================================================
        resultados.sort((a, b) => b.hybridScore - a.hybridScore);
        // ==========================================================
        // TRABALHOS RELACIONADOS
        // ==========================================================
        const idsResultadosPrincipais = resultados.map((item) => Number(item.id));
        const trabalhosRecomendados = await (0, trabalhosRelacionados_1.buscarTrabalhosRelacionados)(queryEmbedding, idsResultadosPrincipais, 5);
        // ==========================================================
        // 9. PAYLOAD
        // ==========================================================
        const payload = {
        ...
        // ==========================================================
        try {
            await redis_1.redis.set(cacheKey, JSON.stringify(payload), {
                EX: 60,
            });
        }
        catch (error) {
